...
class KwhService {
  ...
  Future<List<Map<String, dynamic>>> getHistoryData() async {
    try {
      ...
      // Fetch all devices and filter by mntTag to build history records
  ...
  Future<Map<String, dynamic>> verifyConnection() async {
    try {
      ...
      // Try to fetch data from Firebase (iterate all devices and filter by mntTag)
      final snapshot = await _database.child('deviceEnergyData').get();
      
      if (!snapshot.exists) {
        return {
          'success': false,
          'error': 'No device data found at deviceEnergyData',
      ...
      if (data == null) {
        return {
          'success': false,
          'error': 'Data is null at deviceEnergyData',
          'serviceTag': serviceTag,
          'cleanServiceTag': cleanServiceTag,
        };
      }
